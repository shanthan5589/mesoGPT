...
from mesoGPT.dataset import create_dataloader
...
def train(model, tokenizer, optimizer, criterion,
          ...
          num_workers, run_dir, args, scalar, profiling_enabled=False, performance_timer=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--profiling_enabled", type=parse_bool,  help="Enable profiling during benchmarking.")
    parser.add_argument("--performance_timer", type=parse_bool,  help="Enable performance timing for each optimizer step and evaluation.")
    ...
    if args.profiling_enabled and not torch.cuda.is_available():
        parser.error(
            "Profiling can only be used when CUDA is available."
        )
    ...
    train(
        ...
        performance_timer=args.performance_timer,
    )