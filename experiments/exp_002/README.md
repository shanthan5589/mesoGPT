...
| Model's FLOP rate | ~15.3 TFLOP/s (4.22% MFU) |
...
## Note:
The reported throughput, MFU, and other metrics were calculated using the measured wall clock time which is the total time taken for the entire training process, including time spent on training and time spent on evaluation as well.


## Findings

1. **Model FLOP Utilization is low.** Despite a 96.4% average GPU-utilization reading, the MFU was only 4.22%, indicating substantial headroom in increasing arithmetic throughput.
